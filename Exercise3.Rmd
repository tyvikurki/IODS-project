...
join_cols <- setdif(colnames(por), free_cols)
...
gather(alc2) %>% ggplot(aes(value)) + facet_wrap("key", scales = "free") +
geom_bar()
...
library(readr)
alc2 <- read_csv("https://raw.githubusercontent.com/KimmoVehkalahti/Helsinki-Open-Data-Science/master/datasets/alc.csv", show_col_types=FALSE)
```
...
glm1 <- glm(high_use ~ failures + absences + sex, data = alc, family = "binomial")
...
# find the model with glm()
m <- glm(high_use ~ failures + absences + sex, data = alc2, family = "binomial")
...
# fit the model
model <- glm(high_use ~ failures + absences + sex, data = alc2, family = "binomial")
...
select(alc2, failures, absences, sex, high_use, probability, prediction) %>% tail(10)

# tabulate the target variable versus the predictions
table(high_use = alc2$high_use, prediction = alc$prediction)
